...
export default function ProductPersonalDetails(props) {
  ...
  const formatKeyFeatures = (keyFeatures) => {
    ...
    const textWithMarkers = keyFeatures
      .replace(/<b>(.*?)<\/b>/g, '**$1**')
    ...
    return cleanText
      ...
      .map(feature => {
        const parts = feature.trim().split(/(\*\*.*?\*\*)/g);
              ...
              <div className="card-body p-3">
                {profile[0]?.KeyFeatures ? (
                  <ul className="list-unstyled mb-0">
                    {profile[0].KeyFeatures.split('</p>').filter(item => item.trim()).map((feature, index, array) => (
                      <li key={index}>
                        <div className="d-flex align-items-start mb-2">
                          <i className="bi bi-check-circle-fill text-success me-2 mt-1"></i>
                          <div dangerouslySetInnerHTML={{ __html: feature + (feature.includes('</p>') ? '' : '</p>') }} />
                        </div>
